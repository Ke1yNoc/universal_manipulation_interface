...
class PiperInterpolationControllerIK(mp.Process):
    ...
    def run(self):
        ...
        if self.go_zero_on_start:
            robot.JointCtrl(0, 24000, -55000, 0, 38000, 0)
        ...
        try:
            ...
            try:
                ## Soft release Agilex Piper ##
                robot.ModeCtrl(0x01, 0x01, 10, 0x00)
                robot.JointCtrl(0, 0, 0, 0, 17900, 0)
                time.sleep(10)
                while(robot.DisablePiper()):
                    time.sleep(0.01)
            except Exception:
                pass
            self.ready_event.set()
